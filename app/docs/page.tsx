    ...
    <main className="page-shell">
      ...
      <div className="container docs-layout">
        ...
        <article>
          ...
          <section id="protocol" className="doc-section"><h2>Protocol</h2><p>Current deployment: <span className="mono">eventum/1.1.1</span> on Studionet chain <span className="mono">61999</span>. Snapshot registration is source-bound: GenLayer derives the canonical event identity and authoritative <span className="mono">source_evidence_hash</span> during read-back. The submitted source hash and canonical hint are inputs, not final provenance.</p><h3>Snapshot before judgment</h3><p>A snapshot includes the provider market ID, canonical URL, title, description, outcome labels, resolution rules, authority, time fields, clarifications, retrieval timestamp, submitted source hash, contract-derived evidence hash, and normalized semantic facts. A changed settlement source is a new version.</p><h3>Relationships</h3><p>The contract accepts eleven bounded relations: EQUIVALENT, CONDITIONAL_EQUIVALENT, SUBSET, SUPERSET, OVERLAPPING, CONFLICTING, TEMPORAL_MISMATCH, SOURCE_MISMATCH, OUTCOME_MISMATCH, UNRELATED, and AMBIGUOUS.</p><p>For shared evidence, classification first rejects unrelated or insufficient material. It then tests equivalence, directional containment, explicit conditional equivalence, partial overlap, and genuine conflict. A higher BTC upper-bound threshold contains a lower one; it is a SUPERSET, not CONFLICTING merely because the two rules can settle differently.</p><pre className="code-block">{["{", "  relation: \"TEMPORAL_MISMATCH\",", "  safe_to_compare: false,", "  safe_to_aggregate: false,", "  outcome_mapping: {},", "  reason_codes: [\"TIME_WINDOW_CONFLICT\"],", "  material_differences: [\"…\"],", "  evidence_hashes: [\"…\", \"…\"]", "}"].join("\n")}</pre></section>
